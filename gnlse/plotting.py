...
def plot_delay_vs_distance(solver, time_range=None, ax=None, norm=None):
    ...
    ax.pcolormesh(solver.t, solver.Z, lIT, shading="interp", vmin=-40,
                  cmap="magma")
    ax.set_xlim(time_range)
    """
    interpolator = interp2d(solver.t, solver.Z, lIT)
    newT = np.linspace(np.min(solver.t), np.max(solver.t), lIT.shape[1])
    toshow = interpolator(newT, solver.Z)

    ax.imshow(toshow, origin='lower', aspect='auto', cmap="magma",
              extent=[time_range[0], time_range[1], 0, np.max(solver.Z)],
              vmin=-40)
    """
